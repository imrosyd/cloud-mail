...
const emailService = {
	...
	async send(c, params, userId) {
		...
		let {
			...
			name, //sender name
			sendType, //send type
			emailId, //email id, included for reply
		...
		//If not admin and role has send count limit
		...
		//If external recipients exist but no sending service configured
		...
		//If external recipients exist, prefer Cloudflare Email Service, otherwise use Resend
		if (!allInternal) {
		...
		//If role has send count limit, increment user send count
	...
	async HandleOnSiteEmail(c, receiveEmail, sendEmailData, attList) {
		...
		for (const email of receiveEmail) {
			...
			//Change sender email to recipient
			...
			//If recipient exists, assign email to them
			if (accountRow) {
				...
				//If recipient lacks domain permission or has email blocking, mark as bounced
		...
		let message = ''
		//If any email was bounced, update sender's email status
		if (bouncedEmail) {
