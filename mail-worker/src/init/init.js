...
const dbInit = {
	...
	async v1_6DB(c) {
		...
		const noticeContent = 'This project is for learning and communication purposes only. Illegal use is prohibited.\n' +
			'<br>\n' +
	...
	async v1_5DB(c) {
		...
		try {
			...
			console.warn(`Skipping field add: ${e.message}`);
		}
	},
    ...
    `).run();
        ...
        (36,'密钥删除', 'reg-key:delete', 33, 2, 2)`).run();
			...
			console.warn(`Skipping data: ${e.message}`);
		}
